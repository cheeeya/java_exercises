...
public class DistributionSorts {
	...
	public static void countingSort(int[] arr) {
		...
		int start = 0;
		for (int k = 0; k < 10; k++) {
			for (int j = start; j < counts[k] + start; j++) {
				arr[j] = k;
			}
			start += counts[k];
		}
	...
	public static void MSDRadixSortFromDigitInBounds(int[] arr, int digit,
		...
		if (digit >= 0) {
			if (end - start > 1) {
				int[] digitBounds = countingSortByDigitInBounds(arr, digit,
						start, end);
				int last = start;
				for (int i = 0; i < digitBounds.length; i++) {
					if (digitBounds[i] > 0) {
						MSDRadixSortFromDigitInBounds(arr, digit - 1, last,
								start + digitBounds[i]);
						last = start + digitBounds[i];
					}
				}
			}
		}
	...
	private static int[] countingSortByDigitInBounds(int[] arr, int digit,
		...
		for (int i = start; i < end; i++) {
			int value = arr[i];
			value = value / (int) Math.pow(10, digit);
			value = value % 10;
			counts[value] += 1;
			values[i] = value;
			actual[i] = arr[i];
	...
	public static void main(String[] args) {
		...
		int[] arr4 = new int[] { 2299, 1999, 1991, 1996, 1998, 1990, 1994, 1993,
				1992, 1898, 1859, 2000, 2594, 2582 };
