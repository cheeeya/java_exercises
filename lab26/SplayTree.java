...
public class SplayTree<K extends Comparable<K>, V> {
	...
	private void zigZig(TreeNode node) {
		TreeNode parent = node.parent;
		if (parent.parent.myLeft == parent) {
			rotateRight(parent);
			rotateRight(node);
		} else {
			rotateLeft(parent);
	...
	private class TreeNode {
		 ...
		 **/
		public TreeNode(K key, V value, TreeNode parent, TreeNode left,
				TreeNode right) {
			this.key = key;
			this.value = value;
			this.parent = parent;
